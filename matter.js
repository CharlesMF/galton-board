...
<body>
    ...
    <div class="info">Powered by Matter.js Physics Engine</div>
    ...
    <script>
        ...
        // --- 2. 參數設定 (Config) ---
        const pinRows = 14;      // 釘子層數 - 增加以獲得更好的常態分佈
        const pinSize = 2.5;     // 釘子半徑
        const pinGap = 50;       // 釘子間距 - 大幅增加到球的12倍以上
        const startY = 100;      // 第一層釘子的 Y 座標 - 提高讓球直接落入
        const ballSize = 4;      // 球的半徑
        const restitution = 0.05; // 彈力 - 極低彈性
        const friction = 0;      // 摩擦力 - 完全無摩擦
        ...
        // A. 釘子 (Pins/Pegs) - 使用交錯排列
        const pins = [];
        const pinsPerRow = 13; // 每行固定數量的釘子
        
        for (let row = 0; row < pinRows; row++) {
            for (let col = 0; col < pinsPerRow; col++) {
                // 交錯排列：奇數行偏移半個間距
                const offsetX = (row % 2 === 0) ? 0 : pinGap / 2;
                const x = (width / 2) - ((pinsPerRow - 1) * pinGap / 2) + (col * pinGap) + offsetX;
                const y = startY + row * 35; // 垂直間距
        ...
        // B. 收集桶 (Bins/Buckets)
        const binHeight = 200;
        const binY = height - (binHeight / 2) + 30;
        const binWidth = 3; // 隔板厚度 - 更薄
        const bins = [];
        
        // 根據釘子排列決定桶子數量
        const numBins = pinsPerRow + 1;
        const binStartX = (width / 2) - ((pinsPerRow - 1) * pinGap / 2) - (pinGap / 2);
...
// 左右邊界牆，防止球從側邊逃出
        ...
        function spawnBall() {
            // 加入適當的隨機偏移以產生更好的常態分佈
            const xOffset = Common.random(-15, 15);
            const ball = Bodies.circle(width / 2 + xOffset, 20, ballSize, {
                restitution: restitution, // 彈性
                friction: friction,
                frictionAir: 0.01, // 空氣阻力
                density: 0.001, // 密度 - 降低讓球更輕盈
                slop: 0.05, // 允許輕微重疊不產生推力
                render: {
